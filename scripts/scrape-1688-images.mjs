// Scrape main-gallery product images from 1688 offer links in catalog-master.json
...
// user to solve it manually in their Chrome window, then continues.
//
// Usage: node scripts/scrape-1688-images.mjs [--limit N] [--offset N] [--all]
//   default: process SKUs with a 1688 link AND hasImage=false (the 153 that need images)
//   --all : include SKUs that already have images too (219)
...
if (!ALL) list = list.filter(p => !p.hasImage);
list = list.slice(OFFSET, OFFSET + LIMIT);
log(`worklist: ${list.length} SKUs (ALL=${ALL})`);
...
for (const p of list) {
  ...
  const offer = p.link1688.split('?')[0];
  ...
  for (const u of urls) {
    ...
    try { const bytes = await download(u, path.join(dir, fn)); n++; imgTotal++; log(`  saved ${fn} (${(bytes / 1024).toFixed(0)}kb)`); }
    catch (e) { log(`  fail ${u.slice(-40)}: ${e.message}`); }
  }
  manifest[sku] = { ok: n > 0, count: n, urls, title: (await page.title()).slice(0, 60), at: new Date().toISOString().slice(0, 16) };
  save();
  done++;
}
