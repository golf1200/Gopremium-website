// ai-concierge.jsx — GO PREMIUM "AI Gift Concierge"
// Type a brief → real AI-generated curated gift-set concepts (window.claude.complete + fallback).
...
/* ---- product mock tile: hints the "AI mockup / your logo on product" idea ---- */
function GpProductMock({ label, tint }) {
  return (
    <div style={{
      position:"relative", height:74, borderRadius:11, overflow:"hidden",
      background:`linear-gradient(135deg, ${tint[0]}, ${tint[1]})`,
      display:"flex", alignItems:"flex-end", padding:"9px 10px",
      border:"1px solid rgba(255,255,255,.18)"
    }}>
      <div style={{
        position:"absolute", top:8, right:8, fontFamily:"var(--gp-font-head)", fontSize:8.5,
        fontWeight:600, letterSpacing:".06em", color:"rgba(255,255,255,.92)",
        background:"rgba(0,0,0,.22)", borderRadius:99, padding:"3px 7px",
...
function GpSpark({ size = 13, style }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className="gp-spark" style={style} aria-hidden="true">
...
/* ---- keyword fallback generator (when AI is unavailable) ---- */
function gpFallback(brief) {
  ...
  if (eco) sets = sets.map((s,i)=> i===0 ? s : s).map(s=>({ ...s, tags: Array.from(new Set(["รักษ์โลก", ...s.tags])).slice(0,3) }));
  return sets.map((s,i)=>({ ...s, priceFrom: lo + i*40, priceTo: hi + i*60, tags: s.tags.slice(0,3) }));
}

async function gpAskAI(brief) {
  if (!(window.claude && typeof window.claude.complete === "function")) {
    await new Promise(r=>setTimeout(r, 900));
    return gpFallback(brief);
  }
  const prompt =
`คุณคือ "AI Gift Concierge" ของ GO PREMIUM บริษัทผลิตของพรีเมียม/ของขวัญองค์กรพรีเมียมในไทย
จากโจทย์ของลูกค้า ให้เสนอ "ชุดของขวัญองค์กร" (gift set) ที่คัดสรรมาแล้ว 3 ชุด
ตอบกลับเป็น JSON ที่ถูกต้องเท่านั้น (ไม่มี markdown, ไม่มีคำอธิบายอื่น): array ของ 3 object
แต่ละ object มี key:
- name: ชื่อเซ็ต ภาษาไทยหรืออังกฤษสั้นๆ ติดหู ไม่เกิน 26 ตัวอักษร
- concept: 1 ประโยคภาษาไทย อธิบายว่าทำไมเซ็ตนี้เหมาะกับโจทย์
- items: array ของ 3-4 ชื่อสินค้าภาษาไทย
- tags: array ของ 1-3 แท็กสั้นภาษาไทย (เช่น "รักษ์โลก","ดูแพง","งานด่วน")
- priceFrom: ตัวเลขราคาเริ่มต้นต่อชุด (บาท)
- priceTo: ตัวเลขราคาสูงสุดต่อชุด (บาท)
  ...
  try {
    ...
    if (Array.isArray(arr) && arr.length) {
      return arr.slice(0,3).map(o=>({
        name:String(o.name||"เซ็ตของขวัญ").slice(0,30),
        concept:String(o.concept||""),
        items:Array.isArray(o.items)?o.items.slice(0,4).map(String):[],
        tags:Array.isArray(o.tags)?o.tags.slice(0,3).map(String):[],
        priceFrom:Number(o.priceFrom)||180, priceTo:Number(o.priceTo)||520,
      }));
    }
...
function GpConcierge({ vibe="safe", onRequestQuote }) {
  ...
  async function run(text) {
    ...
    setUsedAI(!!(window.claude && window.claude.complete));
    const sets = await gpAskAI(brief);
    setResults(sets); setStatus("done");
  }

  // hero input can trigger the concierge
  useEffect(()=>{
  ...
  return (
    <div className="gp-wrap">
      <div style={{
        position:"relative", borderRadius:"var(--gp-radius-lg)", overflow:"hidden",
        background:"linear-gradient(160deg,#16293F 0%, #1F3A5F 55%, #244873 100%)",
        boxShadow:"var(--gp-shadow)", color:"#fff",
      }}>
        <div className="gp-dotgrid on-dark" style={{position:"absolute",inset:0,opacity:.5}}/>
        <div style={{position:"absolute",right:-120,top:-120,width:360,height:360,borderRadius:"50%",
          background:"radial-gradient(circle at 32% 32%, rgba(244,189,68,.55), transparent 62%)",opacity:.5,pointerEvents:"none"}}/>
        <div style={{position:"relative", padding:"clamp(26px,4vw,46px)"}}>
          <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
            ...
            <span style={{fontSize:13,color:"#9FB2CC"}}>{usedAI ? "ขับเคลื่อนด้วย AI · ตอบทันที" : "โหมดสาธิต"}</span>
          </div>
          ...
          <div style={{
            ...
            borderRadius:16, padding:10, backdropFilter:"blur(8px)"
          }}>
            ...
            <button className="gp-btn gp-btn-primary gp-btn-lg" onClick={()=>run()} disabled={status==="loading"}
              style={{flex:"0 0 auto", opacity: status==="loading"?.85:1}}>
              {status==="loading"
          ...
          {status==="idle" && (
            <div style={{marginTop:14,display:"flex",gap:9,flexWrap:"wrap",alignItems:"center"}}>
              ...
              {GP_EXAMPLES.map((ex,i)=>(
                <button key={i} onClick={()=>run(ex)} style={{
                  fontFamily:"var(--gp-font-body)", fontSize:12.5, color:"#D8E2F0", cursor:"pointer",
                  background:"rgba(255,255,255,.06)", border:"1px solid rgba(255,255,255,.16)",
                  borderRadius:99, padding:"7px 13px", transition:".15s"
                }}
          ...
          {status==="done" && (
            <div style={{marginTop:24}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(248px,1fr))",gap:16}}>
                {results.map((r,i)=>(
                  <div key={i} style={{
                    background:"#fff", color:"var(--gp-ink)", borderRadius:16, padding:18,
                    display:"flex", flexDirection:"column", boxShadow:"0 16px 40px -18px rgba(0,0,0,.5)"
                  }}>
                    <GpProductMock label={r.items[0]||r.name} tint={GP_TINTS[i*2 % GP_TINTS.length]}/>
...
Object.assign(window, { GpConcierge, GpSpark });
